...
public class ExtendedImageRegistry {
	...
	protected static Object resourceURL = EMFEditPlugin.INSTANCE
			.getImage("full/obj16/Resource");

	protected static String resourceURLPrefix = resourceURL.toString() + "#";

	protected static String itemURLPrefix = EMFEditPlugin.INSTANCE.getImage(
			"full/obj16/Item").toString()
			+ "#";

	protected static String createChildURLPrefix = EMFEditPlugin.INSTANCE
				...
				if (object instanceof ImageDescriptor) {
					...
					if (urlString.startsWith(resourceURLPrefix)) {
						if (EMFPlugin.IS_RESOURCES_BUNDLE_AVAILABLE) {
							...
							System.out
									.println("E4MF: PlatformUI dependency, not vailable");

							// imageDescriptor =
							// PlatformUI.getWorkbench().getEditorRegistry().getImageDescriptor
							// ("dummy." +
							// urlString.substring(resourceURLPrefix.length()));
						} else {
...
class URLImageDescriptor extends ImageDescriptor {
	...
	public Image createImage(boolean returnMissingImageOnError, Device device) {
		// Don't call super because it loses the transparency information if we
